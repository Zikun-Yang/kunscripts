...
if(FALSE){
  ...
  rmarkdown::render("DESeq2.Rmd", output_file = "results/DEG_analysis_by_DESeq2.html")
}
```
...
library(pheatmap)  |> suppressPackageStartupMessages()
...
res <- results(dep)
diff <- res
diff <- na.omit(diff)  # remove missing values
dim(diff)
deg <- as.data.frame(diff)
#deg$log2FoldChange <- -deg$log2FoldChange ## 对于log2FoldChange 列取负数

# mark
deg$change <- as.factor(ifelse(
                  deg$padj < padj_cutoff & abs(deg$log2FoldChange) > logFC_cutoff,
...
max_log2FoldChange <- max(abs(deg$log2FoldChange))

fig = ggplot(data = deg, aes(x = log2FoldChange,y = -log10(padj), color = change))+
  geom_point(alpha = 0.4, size = 1, shape = 16) +
  coord_cartesian(xlim = c(-max_log2FoldChange, max_log2FoldChange)) +
...
fig = ggplot(deg_with_highlight, aes(x = log2FoldChange, y = -log10(padj), label = symbol)) +
